...
void recombineS(PGraphics a, PGraphics b) {
  ...
  for (int i=0; i<objects; i++) {
    //println("recombine",i);
     int r2 = color(a.pixels[i]);
     float r1 = r2;
     //if(alpha(r2)<255)r1 = ((r2+color(0,0,0,128)));
     //else 
     r1 = red(r2)*256*256+green(r2)*256+blue(r2);;
    ...
    if (frameCount%10==0&&i==0)println("col********", num[i],r1,"r2",r2,r,g,b2,b3);
    if (frameCount%10==0&&i==0)println("col2",num[i], t,red(t1),green(t1),blue(t1));
    if (frameCount%10==0&&i==0)println("num", num[i], t);
  }
  //imga.set(0,c1);
  //imgb.set(0,c2);
...
void debug() {
  if (frameCount%10==0) {
    ...
    println("print imgxa", num[0], red(imga.get(0).pixels[0]), green(imga.get(0).pixels[0]), blue(imga.get(0).pixels[0]),"alpha", alpha(imga.get(0).pixels[0]), brightness(imga.get(0).pixels[0]));
    println("print imgxb", num[0], red(imgb.get(0).pixels[0]), green(imga.get(0).pixels[0]), blue(imgb.get(0).pixels[0]),"alpha", alpha(imgb.get(0).pixels[0]), brightness(imgb.get(0).pixels[0]));
    println("print imgya", num[0], red(imga.get(1).pixels[0]), green(imga.get(1).pixels[0]), blue(imga.get(1).pixels[0]),"alpha", alpha(imga.get(1).pixels[0]), brightness(imga.get(1).pixels[0]));
    println("print imgyb", num[0], red(imgb.get(1).pixels[0]), green(imgb.get(1).pixels[0]), blue(imgb.get(1).pixels[0]),"alpha", alpha(imgb.get(1).pixels[0]), brightness(imgb.get(1).pixels[0]));
    println("print c", num[0], red(c1.pixels[0]), green(c1.pixels[0]), blue(c1.pixels[0]),"alpha", alpha(c1.pixels[0]), brightness(c1.pixels[0]));
    println("print c2", num[0], red(c2.pixels[0]), green(c2.pixels[0]), blue(c2.pixels[0]),"alpha", alpha(c2.pixels[0]), brightness(c2.pixels[0]));
  }
};
...
void debugF() {
  if (frameCount%10==0) {
    for (int i=0; i<vars; i++) {
      println("print imga "+i, num[0], red(imga.get(i).pixels[0]), green(imga.get(i).pixels[0]), blue(imga.get(i).pixels[0]),"alpha", alpha(imga.get(i).pixels[0]), brightness(imga.get(i).pixels[0]));
      println("print imgb "+i, num[0], red(imgb.get(i).pixels[0]), green(imgb.get(i).pixels[0]), blue(imgb.get(i).pixels[0]),"alpha", alpha(imgb.get(i).pixels[0]), brightness(imgb.get(i).pixels[0]));
    }
    println("print c", num[0], red(c1.pixels[0]), green(c1.pixels[0]), blue(c1.pixels[0]),"alpha", alpha(c1.pixels[0]), brightness(c1.pixels[0]));
    println("print c2", num[0], red(c2.pixels[0]), green(c2.pixels[0]), blue(c2.pixels[0]),"alpha", alpha(c2.pixels[0]), brightness(c2.pixels[0]));
  }
...
void numToR2() {
  ...
  for (int i=0; i<vars; i++) {
    ...
    for (int j=0; j<imgb.get(0).pixels.length; j++) {
      float k = j+random(cwidth, cwidth*cheight)/10;
      color col = float2PixelsA(k);
      //color col1 = float2PixelsB(k);
      //color col = color(0,2,20+round(random(-10,10)),128);
      color col1 = float2PixelsB(k);
      imga.get(i).pixels[j]= col;
      imgb.get(i).pixels[j]= col1;
      if (j==0) {
        println("x "+i, red(col), green(col), blue(col), alpha(col), k,(col+color(0,0,0,128))*2);
        println("xb "+i, red(col1), green(col1), blue(col1), alpha(col1), k,col1 +16777216);
        println("col ", col, col1, imga.get(i).pixels[j], imgb.get(i).pixels[j]);
      }
    }
    //println("num ",imgb.get(i).pixels[0]+16777216+imgb.get(0).pixels[0]+16777216, k);
    imga.get(i).updatePixels();
    imgb.get(i).updatePixels();
  }
};
