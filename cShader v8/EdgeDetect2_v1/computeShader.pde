class cShader {
  ...
  PGraphics c,c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12, c13, c14, c15;
  int cwidth = 200;
  int cheight = 200;
  float []num;
  String[] colRef ;
...
void recombineSD(PGraphics a, PGraphics b) {
  ...
  for (int i=0; i<objects; i++) {
    ...
    imga.get(0).pixels[i] = float2PixelsA(r1);
    imgb.get(0).pixels[i] = float2PixelsB(t);
    if (frameCount%10==0&&i==0)println("col********", num[i],r2,"rid",r1a,r,g,b4,"Alpha",r1a,b3);
    if (frameCount%10==0&&i==0)println("col2",num[i]+t, t,"exc",b.pixels[i]+16777216);
    if (frameCount%10==0&&i==0)println("num", num[i], t);
  }
  //imga.set(0,c1);
  //imgb.set(0,c2);
...
void numToR2() {
  ...
  for (int i=0; i<vars; i++) {
    ...
    for (int j=0; j<imgb.get(0).pixels.length; j++) {
      ...
      color col1 = float2PixelsB(k);
      //float r = 2*256;
      //float g = 20+round(random(-10,10));
      //color col = color(0,0,g,128);
      //color col1 = float2PixelsB(k);
      imga.get(i).pixels[j]= col;
      imgb.get(i).pixels[j]= col1;
      if (j==0) {
        println("x "+i, red(col), green(col), blue(col), alpha(col), k);
        println("xb "+i, red(col1), green(col1), blue(col1), alpha(col1), k);
