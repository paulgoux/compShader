class writeShader {
  ...
  void create(boolean k) {
    ...
    if(!k){
      sk = new String[2];
      sk[0] = "float numf = abs(num - int(num))*1000 ;";
      //ddition
  ...
  String[] sclc (boolean k) {
    ...
    int count = 0;
      ...
      for(int i=0;i<vars;i++){
        ss5[i] = "if(via"+i+".a<1.0)tvf"+i+"= (tvb"+i+"+(-(tva"+i+")*0.0));";
      }
      for(int i=0;i<ss.length;i++){
        ssf[i] = ss[i];
  ...
  String[] shderWrite (boolean k){
      ...
      if(k){
        ss = new String [2];
        ss[0] = "if(numf>0.0)gl_FragColor = vec4(rni, gni, bni, 1.0);"; 
        ss[1] = "else  gl_FragColor = vec4(rni, gni, bni, 0.5);";
      }
      else {
        ss = new String [1];
        ss[0] ="if(numf>0.0)gl_FragColor = vec4(rni, gni, bni, 1.0);"; 
